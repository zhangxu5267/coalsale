package com.drzyl.dict.persistent.impl;
...
import com.drzyl.dict.entity.DictValue;
import com.drzyl.dict.persistent.IDictValuePersistent;
import com.drzyl.dict.query.DictValueQuery;
import com.drzyl.dict.vo.DictValueVO;
 ...
 */
@Log4j2
@Repository("com.drzyl.dict.DictValuePersistent")
public class DictValuePersistentImpl extends ToolPersistent implements IDictValuePersistent {
  ...
  public Long getCountDictValue(DictValueQuery dictValueQuery) throws Exception {
    ...
    try {
      ...
      if (!ToolUtil.isNullEntityAllFieldValue(dictValueQuery)) {
        this.getExistsSql(dictValueQuery);
  ...
  private void getExistsSql(DictValueQuery dictValueQuery) throws Exception {
    try {
      if (ToolUtil.isNotNullStr(dictValueQuery.getToolExists())) {
        if ("NEXUS".equals(dictValueQuery.getToolExists().toUpperCase())) {
          dictValueQuery.setToolExists("SELECT NEXUS_ID FROM NEXUS WHERE NEXUS_ID IS NOT NULL AND ID = " + TABLE_ALIAS + ".ID");
        } else {
          dictValueQuery.setToolExists(null);
        }
      }
      if (ToolUtil.isNotNullStr(dictValueQuery.getToolNotExists())) {
        if ("NEXUS".equals(dictValueQuery.getToolNotExists().toUpperCase())) {
          dictValueQuery.setToolNotExists("SELECT NEXUS_ID FROM NEXUS WHERE NEXUS_ID IS NOT NULL AND ID = " + TABLE_ALIAS + ".ID");
        } else {
          dictValueQuery.setToolNotExists(null);
        }
      }
    } catch (Exception e) {
      if (log.isErrorEnabled()) {
