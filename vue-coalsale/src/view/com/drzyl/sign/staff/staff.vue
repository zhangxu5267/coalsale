...
<template>
  <div>
    <Card>
      ...
      <Button class="pro-list-button" type="info" icon="ios-link-outline" ghost @click="createPostStaffNexus()" v-if="staffSelectRowLength === 1">关联岗位</Button>
        ...
        <Form class="pro-margin-top-20" ref="staffForm" :model="staff" :rules="staffFormValidate" :label-width="120">
          <Row type="flex" justify="center">
            ...
            <Col :xs="24" :sm="20" :md="18" :lg="14">
              ...
              <Dict v-model="staff.gender" dictKey="SYS_GENDER" type="radio" formItemProp="gender" />
            </Col>
            <Col :xs="24" :sm="20" :md="18" :lg="14">
              <FormItem label="紧急联系人姓名：" prop="emergencyContactName">
            ...
            <Col :xs="24" :sm="20" :md="18" :lg="14">
              <FormItem label="描述：" prop="description">
                <Input v-model="staff.description" type="textarea" :rows="3" placeholder="输入描述"></Input>
              </FormItem>
            </Col>
            <Col :xs="24" :sm="20" :md="18" :lg="14">
              <FormItem>
      ...
      <Card>
        ...
        <Form class="pro-margin-top-20" :label-width="120">
          <Row type="flex" justify="center">
            ...
            <Col :xs="24" :sm="20" :md="8" :lg="8">
              <FormItem label="手机号：">
                {{ staff.mobilePhoneNumber }}
              </FormItem>
            </Col>
            <Col :xs="24" :sm="20" :md="18" :lg="18">
              <FormItem label="岗位名称：">
                <Select v-model="postNameList" clearable filterable multiple @on-change="changePostNameListValue($event)">
                  <Option v-for="item in postSetOfPostStaffNexusEdit" :value="item.postId" :key="item.postId">{{ item.postName + ' [ ' + item.namePath + ' ]' }}</Option>
                </Select>
              </FormItem>
            </Col>
            <Col :xs="24" :sm="20" :md="18" :lg="18">
              <FormItem label="默认岗位：">
                <Select v-model="defaultPostName" clearable filterable>
                  <Option v-for="item in defaultPostNameList" :value="item.postId" :key="item.postId">{{ item.postName + ' [ ' + item.namePath + ' ]' }}</Option>
                </Select>
              </FormItem>
            </Col>
            <Col :xs="24" :sm="20" :md="18" :lg="12">
              <FormItem>
                <Button class="pro-margin-right-8" @click="postStaffNexusEditToStaffList()">取消</Button>
...
export default {
  data () {
    return {
      ...
      postNameList: [],
      postSetOfPostStaffNexusEdit: [],
      defaultPostName: null,
      defaultPostNameList: []
    }
  },
  computed: {
  ...
  methods: {
    ...
    ...mapActions('postStaffNexusStore', [
      'batchSavePostStaffNexus',
      'removePostStaffNexus',
      'queryPostStaffNexus',
    ...
    delStaff () {
      ...
      let delPostStaffNexusList = []
      this.staffSelectRow.forEach(staff => {
        let postStaffNexus = new PostStaffNexus()
        postStaffNexus.staffId = staff.staffId
        delPostStaffNexusList.push(postStaffNexus)
      })
      this.batchRemovePostStaffNexus(delPostStaffNexusList).then(res => {
        if (res.data.success) {
    ...
    createPostStaffNexus () {
      this.postStaffNexusEditModalShow = true
      this.getAllPostVO().then(res => {
        if (res.data.success) {
          this.postSetOfPostStaffNexusEdit = res.data.data
          let postStaffNexusQuery = new PostStaffNexusQuery()
          postStaffNexusQuery.staffId = this.staffSelectRow[0].staffId
          this.queryPostStaffNexus(postStaffNexusQuery).then(res => {
            if (res.data.success) {
              this.resetPostStaffNexusForm()
              res.data.data.forEach(postStaffNexus => {
                this.postNameList.push(postStaffNexus.postId)
                this.defaultPostNameList.push(this.postSetOfPostStaffNexusEdit.find(e => e.postId === postStaffNexus.postId))
                if (postStaffNexus.defaultPost === 'y') {
                  this.defaultPostName = postStaffNexus.postId
                }
              })
              let staffEdit = new Staff()
              staffEdit.staffId = this.staffSelectRow[0].staffId
    ...
    postStaffNexusEditToStaffList () {
      this.postStaffNexusEditModalShow = false
      this.getStaffSet()
    },
    resetPostStaffNexusForm () {
      this.postNameList = []
      this.defaultPostName = null
      this.defaultPostNameList = []
    },
    savePostStaffNexusForm () {
      this.postStaffNexusSubmitting = true
      let postStaffNexusList = []
      this.postNameList.forEach(postName => {
        let postStaffNexus = new PostStaffNexus()
        postStaffNexus.staffId = this.staff.staffId
        postStaffNexus.postId = postName
        if (this.defaultPostName && this.defaultPostName === postName) {
          postStaffNexus.defaultPost = 'y'
        }
        postStaffNexusList.push(postStaffNexus)
      })
      if (postStaffNexusList.length > 0) {
        let delPostStaffNexus = new PostStaffNexus()
        delPostStaffNexus.staffId = this.staff.staffId
        this.removePostStaffNexus(delPostStaffNexus).then(res => {
          if (res.data.success) {
            this.batchSavePostStaffNexus(postStaffNexusList).then(res => {
              if (res.data.success) {
                this.$Message.success({ content: '岗位员工信息保存成功.', duration: 3 })
                this.resetPostStaffNexusForm()
                this.staff = new Staff()
                this.postStaffNexusSubmitting = false
                this.postStaffNexusEditToStaffList()
              } else {
                this.$Message.error({ content: res.data.msg, duration: 6 })
    ...
    changePostNameListValue (value) {
      if (value.length > 0) {
        this.defaultPostNameList = []
        value.forEach(postName => {
          this.defaultPostNameList.push(this.postSetOfPostStaffNexusEdit.find(e => e.postId === postName))
        })
      } else {
        this.defaultPostNameList = []
      }
