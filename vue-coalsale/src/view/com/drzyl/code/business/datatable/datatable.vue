      ...
      <Card>
        ...
        <Form class="pro-margin-top-20" ref="dtForm" :model="dt" :rules="dtFormValidate" :label-width="120">
          <Row type="flex" justify="center">
            ...
            <Col :xs="24" :sm="20" :md="18" :lg="24" v-if="dataBaseType === 'mysql'">
            ...
            <Col :xs="24" :sm="20" :md="18" :lg="24" v-if="dataBaseType === 'oracle'">
              <FormItem label="数据库表名：" prop="dtName">
                <Select v-model="dt.dtName" clearable filterable>
                  <Option v-for="item in allTableDataSet" :value="item.tableName" :key="item.tableName">{{ item.tableName + ' [ ' + item.comments + ' ]' }}</Option>
                </Select>
              </FormItem>
            </Col>
...
import Dt from '@/view/com/drzyl/code/dt/Dt'
import DtQuery from '@/view/com/drzyl/code/dt/DtQuery'
import Sort from '@/view/com/drzyl/code/sort/Sort'
import SortQuery from '@/view/com/drzyl/code/sort/SortQuery'
import SortVO from '@/view/com/drzyl/code/sort/SortVO'
import VirtualColumns from '@/view/com/drzyl/code/virtualColumns/VirtualColumns'
import VirtualColumnsQuery from '@/view/com/drzyl/code/virtualColumns/VirtualColumnsQuery'
import VirtualColumnsVO from '@/view/com/drzyl/code/virtualColumns/VirtualColumnsVO'
import Columns from '@/view/com/drzyl/code/columns/Columns'
import ColumnsQuery from '@/view/com/drzyl/code/columns/ColumnsQuery'
...
export default {
  ...
  methods: {
    ...
    addDt () {
      ...
      this.dt = new Dt()
      this.getDataBaseType().then(res => {
        if (res.data.success) {
          this.dataBaseType = res.data.data[0].dataBaseType
          if (this.dataBaseType === 'oracle') {
            this.getAllTable().then(res => {
              if (res.data.success) {
                this.allTableDataSet = res.data.data
              } else {
                this.$Message.error({ content: res.data.msg, duration: 6 })
              }
            }).catch(result => {
