...
import com.drzyl.sign.vo.SessionVO;
 ...
 */
@Log4j2
@Repository("com.drzyl.sign.SessionPersistent")
public class SessionPersistentImpl extends ToolPersistent implements ISessionPersistent {
  ...
  public Long getCountSession(SessionQuery sessionQuery) throws Exception {
    ...
    try {
      ...
      if (!ToolUtil.isNullEntityAllFieldValue(sessionQuery)) {
        this.getExistsSql(sessionQuery);
  ...
  private void getExistsSql(SessionQuery sessionQuery) throws Exception {
    try {
      if (ToolUtil.isNotNullStr(sessionQuery.getToolExists())) {
        if ("NEXUS".equals(sessionQuery.getToolExists().toUpperCase())) {
          sessionQuery.setToolExists("SELECT NEXUS_ID FROM NEXUS WHERE NEXUS_ID IS NOT NULL AND ID = " + TABLE_ALIAS + ".ID");
        } else {
          sessionQuery.setToolExists(null);
        }
      }
      if (ToolUtil.isNotNullStr(sessionQuery.getToolNotExists())) {
        if ("NEXUS".equals(sessionQuery.getToolNotExists().toUpperCase())) {
          sessionQuery.setToolNotExists("SELECT NEXUS_ID FROM NEXUS WHERE NEXUS_ID IS NOT NULL AND ID = " + TABLE_ALIAS + ".ID");
        } else {
          sessionQuery.setToolNotExists(null);
        }
      }
    } catch (Exception e) {
      if (log.isErrorEnabled()) {
