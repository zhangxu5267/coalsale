 ...
 */
@Log4j2
@Repository("com.drzyl.sign.AccountPersistent")
public class AccountPersistentImpl extends ToolPersistent implements IAccountPersistent {
  ...
  public Long getCountAccount(AccountQuery accountQuery) throws Exception {
    ...
    try {
      ...
      if (!ToolUtil.isNullEntityAllFieldValue(accountQuery)) {
        this.getExistsSql(accountQuery);
  ...
  private void getExistsSql(AccountQuery accountQuery) throws Exception {
    try {
      if (ToolUtil.isNotNullStr(accountQuery.getToolExists())) {
        if ("NEXUS".equals(accountQuery.getToolExists().toUpperCase())) {
          accountQuery.setToolExists("SELECT NEXUS_ID FROM NEXUS WHERE NEXUS_ID IS NOT NULL AND ID = " + TABLE_ALIAS + ".ID");
        } else {
          accountQuery.setToolExists(null);
        }
      }
      if (ToolUtil.isNotNullStr(accountQuery.getToolNotExists())) {
        if ("NEXUS".equals(accountQuery.getToolNotExists().toUpperCase())) {
          accountQuery.setToolNotExists("SELECT NEXUS_ID FROM NEXUS WHERE NEXUS_ID IS NOT NULL AND ID = " + TABLE_ALIAS + ".ID");
        } else {
          accountQuery.setToolNotExists(null);
        }
      }
    } catch (Exception e) {
      if (log.isErrorEnabled()) {
