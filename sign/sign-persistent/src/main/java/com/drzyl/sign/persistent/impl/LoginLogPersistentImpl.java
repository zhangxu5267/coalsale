 ...
 */
@Log4j2
@Repository("com.drzyl.sign.LoginLogPersistent")
public class LoginLogPersistentImpl extends ToolPersistent implements ILoginLogPersistent {
  ...
  public Long getCountLoginLog(LoginLogQuery loginLogQuery) throws Exception {
    ...
    try {
      ...
      if (!ToolUtil.isNullEntityAllFieldValue(loginLogQuery)) {
        this.getExistsSql(loginLogQuery);
  ...
  private void getExistsSql(LoginLogQuery loginLogQuery) throws Exception {
    try {
      if (ToolUtil.isNotNullStr(loginLogQuery.getToolExists())) {
        if ("NEXUS".equals(loginLogQuery.getToolExists().toUpperCase())) {
          loginLogQuery.setToolExists("SELECT NEXUS_ID FROM NEXUS WHERE NEXUS_ID IS NOT NULL AND ID = " + TABLE_ALIAS + ".ID");
        } else {
          loginLogQuery.setToolExists(null);
        }
      }
      if (ToolUtil.isNotNullStr(loginLogQuery.getToolNotExists())) {
        if ("NEXUS".equals(loginLogQuery.getToolNotExists().toUpperCase())) {
          loginLogQuery.setToolNotExists("SELECT NEXUS_ID FROM NEXUS WHERE NEXUS_ID IS NOT NULL AND ID = " + TABLE_ALIAS + ".ID");
        } else {
          loginLogQuery.setToolNotExists(null);
        }
      }
    } catch (Exception e) {
      if (log.isErrorEnabled()) {
